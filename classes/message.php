...
class message {
    ...
    /** Category: the message was composed by a person. */
    const CATEGORY_PRIMARY = 0;

    /** Category: the message was generated by a component and delivered by a message processor. */
    const CATEGORY_UPDATES = 1;
    ...
    /**
     * Frankenstyle component that originated the message, or null if a person composed it.
     *
     * Write-once: it is set by create() and is deliberately absent from the record that
     * update() builds, so that autosaving a draft cannot alter it and no caller can
     * relabel a message after the fact.
     *
     * @var ?string
     */
    ...
    private function __construct(\stdClass $record, course $course) {
        ...
        $this->component = empty($record->component) ? null : (string) $record->component;
    }
    ...
    public static function create(message_data $data): self {
        ...
        $messagerecord->component = $data->component;
        ...
        $userrecord->category = $message->category();
    ...
    public static function get_many(array $ids): array {
        ...
        $fields = 'id, courseid, subject, content, format, attachments, draft, time, component';
     ...
     *
     * The category is a function of the component rather than a stored fact of its own,
     * so that the two can never disagree. Every write site that denormalizes it into
     * local_mail_message_users has to take it from here.
     *
     * @return int One of the CATEGORY_* constants.
     */
    public function category(): int {
        return self::category_from_component($this->component);
    }

    /**
     * Returns the category that a message with the given component belongs to.
     *
     * This is the single place the rule is written down. Restore has to derive the
     * category without a message instance, and any other write site that denormalizes
     * it has to come through here too, so that no copy can encode a different rule.
     *
     * @param ?string $component Frankenstyle component, or null if a person composed the message.
     * @return int One of the CATEGORY_* constants.
     */
    public static function category_from_component(?string $component): int {
        return $component === null || $component === '' ? self::CATEGORY_PRIMARY : self::CATEGORY_UPDATES;
    }
    ...
    public function set_deleted(user $user, int $status): void {
        ...
        if ($fulldelete) {
            ...
            if ($resetmetadata) {
                ...
                /*
                 * Stamp when the message entered the trash, and clear the stamp when it
                 * leaves. The retention sweep reads this rather than the message time,
                 * which is the send time: without it a message would be purged on the age
                 * of its content instead of on how long it has been in the trash.
                 */
                $sql = 'UPDATE {local_mail_message_users}'
                    . ' SET deleted = :deleted, timedeleted = :timedeleted'
                    . ' WHERE messageid = :messageid AND userid = :userid';
                $params = [
                    'messageid' => $this->id,
                    'userid' => $user->id,
                    'deleted' => $status,
                    'timedeleted' => $status == self::DELETED ? time() : 0,
                ];
                $DB->execute($sql, $params);
            }
    ...
    public function update(message_data $data): void {
        ...
        /*
         * Component is absent from this record on purpose. update_record() only emits a
         * SET clause for the properties the record carries, the same mechanism that lets
         * draft survive every autosave, so leaving it out is what makes the provenance
         * write-once rather than something an update could quietly rewrite.
         */
        ...
        $sql = 'UPDATE {local_mail_message_users}'
            . ' SET courseid = :courseid, deleted = :deleted, timedeleted = 0, time = :time'
        ...
        foreach (['to', 'cc', 'bcc'] as $rolename) {
            ...
            foreach ($data->$rolename as $user) {
                ...
                if (!isset($this->roles[$user->id])) {
                    ...
                    $userrecord->starred = 0;
                    $userrecord->deleted = self::NOT_DELETED;
                    $userrecord->category = $this->category();
                    $DB->insert_record('local_mail_message_users', $userrecord);
